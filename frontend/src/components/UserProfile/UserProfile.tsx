...
const UserProfile = () => {
  ...
  const handleAvatarUpload = async event => {
    ...
    try {
      const res = await fetch(
        'http://127.0.0.1:8000/api/v1/profiles/mine/upload_avatar/',
        {
          method: 'POST',
          headers: {
            Authorization: `Token ${localStorage.getItem('authToken') || ''}`,
  ...
  useEffect(() => {
    const shouldShowModal =
      localStorage.getItem('showIncompleteModal') === 'true';
      ...
      <div className="profile-container">
        <div className="profile-left">
          ...
          <div className="profile-info">
            ...
            <p>
              <span>Location:</span> {profileData?.location || 'Not specified'}
            </p>
            <p>
              <span>Email:</span> {email || 'Not logged in'}
            </p>
            <p>
              <span>Phone:</span> {profileData?.phone_number || 'Not provided'}
            </p>
            <p>
              <span>Direction:</span>{' '}
              {profileData?.direction || 'Not specified'}
            </p>
          </div>
        </div>
      ...
      {isProfileIncompleteModalOpen && (
        <div className="profile-incomplete-modal">
          <div className="modal-contents">
            ...
            <p>
              Your profile is incomplete! Please add information in the profile
              settings.
            </p>
            <img
              src="/dream-helper/profile-page/settings.png"
